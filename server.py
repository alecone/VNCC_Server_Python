...
class ClientThread(Thread):
        ...
        if '|' in user_pass:
            ...
            if ok_domain:
                ...
                if ret == NOK:
                    ...
                    self.user_path = ret
                    print('Registration OK, sendig OK. User path is ',
                          self.user_path)
    ...
    def create_volume(self, dir_path):
        ...
        if ret == OK:
            ...
            ret = ret.decode('utf-8')
            if ret == OK:
                # Tell Cindarello the mount point disk
                self.openstack.send(disks[0].encode())
                ret = self.openstack.recv(BUFFER_SIZE)
                ret = ret.decode('utf-8')
                if ret == OK:
                    ...
                    if len(disks) > 0:
                        ...
                        print("Going to mount ",
                              disks[0], " on mount point ", dir_path)
                        sleep(2)
